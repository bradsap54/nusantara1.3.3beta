...
integrate_module() {
    ...
    sudo docker exec -i iriswebapp_db psql -U postgres -d iris_db -c "INSERT INTO user_client (id, user_id, client_id, access_level, allow_alerts) VALUES (1, 1, 1, 4, 't') ON CONFLICT (id) DO NOTHING;"
        ...
        select opt in \
            ...
            case $REPLY in
                1)
                    ...
                    if [ -z "$IP" ]; then
                        echo -e "\e[1;31m[ERROR] Could not determine IP address. Aborting.\e[0m"
                        break
                    fi
                    ...
                    for i in $(seq 1 10); do
                        ...
                        ssh -o BatchMode=yes -o ConnectTimeout=5 "fakeuser@$IP" &>/dev/null
                        sleep 1
                    done
